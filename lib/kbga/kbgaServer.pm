...
our %async_run_methods = (
        'count_contigs_async' => 'kbga.count_contigs',
        'annotate_genome_async' => 'kbga.annotate_genome',
);

our %async_check_methods = (
        'count_contigs_check' => 'kbga.count_contigs',
        'annotate_genome_check' => 'kbga.annotate_genome',
);
...
sub call_method {
    ...
    my $ctx = kbga::kbgaServerContext->new($self->{loggers}->{userlog},
...
{
    # Service kbga requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for kbga but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = kbga::kbgaServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service kbga.");
    }
...
package kbga::kbgaServerContext;

use strict;

=head1 NAME

kbga::kbgaServerContext
...
package kbga::kbgaServerStderrWrapper;
