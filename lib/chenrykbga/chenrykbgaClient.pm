package chenrykbga::chenrykbgaClient;
...
sub new
{
    ...
    my $self = {
	client => chenrykbga::chenrykbgaClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
	...
	method => "chenrykbga.annotate_genome",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for chenrykbga::chenrykbgaClient\n";
    }
    if ($sMajor == 0) {
        warn "chenrykbga::chenrykbgaClient version is $svr_version. API subject to change.\n";
    }
}
...
package chenrykbga::chenrykbgaClient::RpcClient;
